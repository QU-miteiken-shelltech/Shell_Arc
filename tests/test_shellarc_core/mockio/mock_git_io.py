"""
Git_IO の Mock 実装 ―― 更新版。

【設計方針の変更】
以前はbranch/commit/pending状態をインスタンス内の辞書で模擬する簡易版だったが、
「Mock gitはいらず、本番と同等のものを一時ディレクトリに対して使いたい」との指示を受け、
本番の Git_IO と全く同じロジック(実際の `git` コマンドをsubprocessで実行する実装)に
差し替えた。クラス名は既存の呼び出し箇所(emu_backend.py の `from mock_git_io import
Mock_Git_IO`)との互換性のため `Mock_Git_IO` のまま維持している。

【実行環境への要件】
- `git` バイナリがPATH上に必要(実際にsubprocessで呼び出すため)。
- コンストラクタは git_repo_local_dir を渡しても無条件に Cfg_IO() を
  インスタンス化する(渡された元コードのまま、未修正)。Cfg_IO の実装次第では、
  設定が用意されていない実行環境でここが失敗する可能性がある。
- sync_remote() は `git push origin ...` を実行するが、一時リポジトリには
  origin リモートが存在しないため、エミュレータ上では失敗するのが正常な挙動。
"""
